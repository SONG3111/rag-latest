...
- 系统提示词迁移自 LlamaIndex / RAGFlow（见 `backend/app/agent/prompts.py` 模块注释）
...
- Word 标题判定迁移自 docling 的 `msword_backend._get_label_and_level`（见
  `backend/app/retrieval/chunking.py` 的 `_is_heading`）
...
4. **迁移必配回归测试**：用测试钉住迁移来的行为（本项目所有迁移修复都有对应
   回归用例，见 `backend/tests/`、`mcp-office-server/tests/`）。
...
- backend 测试套件（`backend/tests/`，120 个用例）：fake LLM / fake embeddings /
  seed 提案 + 真实 MCP 子进程，全程不调用模型；
- mcp-office-server 测试套件（`mcp-office-server/tests/`，74 个用例）：直接调用
  工具函数与 MCP tool 层；
- 需要多提案审批顺序等 API 级场景时，用 seed 数据直接驱动 REST 接口
  （参考 `backend/tests/test_proposal_rebase.py`、`test_api_operations.py` 的写法）。
